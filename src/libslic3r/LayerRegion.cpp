...
Flow LayerRegion::bridging_flow(FlowRole role, bool force_thick_bridges) const
{
    ...
    const unsigned int       extruder_id    = region.extruder(role) - 1;

    // Resolve the per-filament bridge flow ratio override for the active extruder.
    const ConfigOptionFloat bridge_flow_ratio = resolve_filament_override(
        print_config, "filament_bridge_flow_ratio", extruder_id, region_config.bridge_flow_ratio);
    ...
    if (print_object.config().thick_bridges || force_thick_bridges) {
        ...
        return Flow::bridging_flow(float(sqrt(bridge_flow_ratio.value)) * nozzle_diameter, nozzle_diameter);
    } else {
        // The same way as other slicers: Use normal extrusions. Apply bridge_flow_ratio while maintaining the original spacing.
        return this->flow(role).with_flow_ratio(bridge_flow_ratio.value);
    }
}
